...
>
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg mx-4 md:mx-auto overflow-auto max-h-screen">
    ...
    <form id="departmentForm" enctype="multipart/form-data">
      ...
      <div class="mb-4">
        <label for="departmentDean" class="block text-gray-700 mb-2">Department Chairperson</label>
        <select id="departmentDean"
          name="departmentDean"
        class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-green-500 "
        >
        <option value="">Select Chairperson</option>
        <?php 
        $rows = $db->getUsers('department_chairperson');
        ...
        foreach ($rows as $row) {
          ...
          $added = $row['added'];
            if($added == "0")
            {
              ...
              echo '<option hidden value="'.$id.'">'.$name.'</option>';
            }      
          }
        ?>
        </select>
      ...
      <!-- Department Head Input -->
      <div class="mb-4">
        <label for="departmentSecretary" class="block text-gray-700 mb-2">Department Secretary</label>

        <select id="departmentSecretary"
          name="departmentSecretary"
        class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-green-500 "
        >
        <option value="">Select Department Secretary</option>
        <?php 
        $rows = $db->getUsers('department_secretary');
...
<script>
  $('.editBtnDepartment').click(function(){
    ...
    $('#modal').removeClass('hidden')
          ...
          var department_secretary = ($(this).data('department_secretary'))
          var department_code = ($(this).data('department_code'))
          var department_dean = ($(this).data('department_dean'))
          ...
          $('#departmentSecretary option').each(function() {
            if($(this).val()===department_secretary){
              $(this).removeAttr('hidden'); // Remove the hidden attribute to ensure options are visible
            };  
          });

          $('#departmentDean option').each(function() {
            if($(this).val()===department_dean){
              $(this).removeAttr('hidden'); // Remove the hidden attribute to ensure options are visible
            };
          });


          $('#departmentSecretary').val(department_secretary)
          $('#departmentDean').val(department_dean)
