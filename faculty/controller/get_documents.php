...
if (
    ...
    $where = array(
     ...
     $faculty_checker_subject = $db->getIdByColumnValueWhere("faculty_subject_section", $where, 'subject_id');
    ...
    if(count($documentRows) > 0)
    {
        foreach ($documentRows as $row) {
            ...
            // CHECKING IF HANDLE DIN NYA YUNG SUBJECTS IF NOT
            if($faculty_checker_subject=="")
            {
                //  CONTINUE IF HINDI SYLLABUS
                if($upload_name!="Syllabus Acceptance")
                {
                    continue;
                }
            }
