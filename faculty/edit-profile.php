...
?>
    ...
    <div class="container mx-3 p-2 bg-white">
        ...
        <div class="flex">
            <button type="submit" class="bg-green-700  text-white btnSaveProfile py-2 px-4 rounded-lg font-semibold hover:bg-green-500  transition">
                Save Changes
                    ...
                    <div  class="grid p-3  gap-x-10  lg:grid-cols-1">

                    <div class="">
                        <p class="text-gray-400 text-center sm:text-left">Documents</p>
                        <div style="height:600px" class="card bg-white items-center flex flex-col shadow-md rounded-lg p-10 ">
                                <?php echo $file_object; ?>

                                <p class="text-black my-5 ">Resume</p>
                                <div class="grid w-full ">
                                    <p class="text-sm text-right ">
                                        <a href="#" id="modalUploadBtn" class="bg-orange mt-3 text-center w-full font-semibold text-orange px-3 py-3 rounded-full shadow-lg transform transition-transform duration-200 hover:scale-105 hover:shadow-xl hover:bg-orange-600">Upload</a>
                                    </p>
                                </div>
                        </div>
                    </div>
                ...
                <div class="overflow-y-scroll px-4 lg:mt-0 mt-3 pb-4" style="height:500px " hidden>

                    <div class="grid lg:grid-cols-2 items-center grid-cols-1 gap-5">
                        <p class="text-gray-400 text-center">Subjects</p>
     ...
     $image = $db->getIdByColumnValue('subject','id',$row['subject_id'],'image');
                            ...
                            $faculty_subject_section_rows = $db->getAllRowsFromTableWhere("faculty_subject_section",['faculty_id='.$user_id,'subject_id='.$row['subject_id']]);

                            $checkerSectionRow = count($faculty_subject_section_rows);

                            $sectionChecker = $checkerSectionRow==0 ? '
                            <button '.$data.'
                                            class="bg-orange mt-3 mb-2 btnSectionAdd text-orange px-1 py-2 rounded-full shadow-lg transform transition-transform duration-200 hover:scale-105 hover:shadow-xl hover:bg-orange-600">
                                            Add Sections
                                            </button>
                            ':'
                            <button '.$data.'
                                            class="bg-green-300 mt-3 mb-2 btnSectionUpdate text-orange px-1 py-2 rounded-full shadow-lg transform transition-transform duration-200 hover:scale-105 hover:shadow-xl hover:opacity-75">
                                            Update Sections
                                            </button>
                            ';
                            ...
                            <div class="card bg-white shadow-md flex flex-col rounded-lg p-6">
                                ...
                                <p class="flex"><span class="font-bold whitespace-nowrap truncate md:truncate">' . htmlspecialchars(ucwords($name)) . '</span></p>
                                <p><span class="font-bold whitespace-nowrap">' . htmlspecialchars(strtoupper($code)) . '</span></p>
                                    '.$sectionChecker.'
                                    <a href="subject-info.php?s='.$row['subject_id'].'"
                                    class="bg-orange text-center w-full text-orange px-1 py-2 rounded-full shadow-lg transform transition-transform duration-200 hover:scale-105 hover:shadow-xl hover:bg-orange-600">
                                    Open
                                    </a>
                        ...
                        echo '
                        <div class="card bg-gray-100 shadow-md flex flex-col items-center justify-center rounded-lg p-6 col-span-3">
